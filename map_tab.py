...
# Individual type colors based on temperament
type_colors = {
...
variant_colors = {
    'A': '#9C27B0',  # Purple for Assertive
    'T': '#FF5722'   # Deep Orange for Turbulent
}

# Define temperament groups
temperament_groups = {
...
# Helper function to get temperament for a type
...
# Helper function to get country code for mapping
...
def load_and_process_data(countries_path, types_path):
    ...
    for _, row in countries_df.iterrows():
        ...
        for col in mbti_cols:
            ...
            # Add to temperament groups
            temp = get_temperament(base_type)
            if temp:
                temperaments[temp] += value
        
        # Find dominant types and temperaments
        ...
        # Add individual type percentages as columns
        for t, v in type_percentages.items():
            result[f'type_{t.lower()}'] = v
        
        results.append(result)
    
    # Process types data for info
    types_info = {}
...
def create_world_map(df, color_by='dominant_temperament', selected_country=None):
    ...
    # Highlight selected country if specified
    if selected_country:
        ...
        if not selected_row.empty:
            ...
            # Add a trace to highlight the selected country
            fig.add_trace(
                go.Choropleth(
...
# Create temperament distribution visualization
...
# Main function to show the map tab
def show_map_tab():
    ...
    # Create loading spinner while data loads
    ...
    with col1:
        ...
        # Convert to parameter for function
    ...
    if country_select:
        ...
        # Display country information in 3 columns
        ...
        with col1:
            ...
            st.markdown(f"<h2 style='text-align: center; color: {type_colors.get(dominant_type, '#808080')};'>{dominant_type}</h2>", unsafe_allow_html=True)
            
            if dominant_type in types_info:
                st.markdown(f"<p style='text-align: center;'><i>{types_info[dominant_type]['nickname']}</i></p>", unsafe_allow_html=True)
        ...
        with col3:
            ...
            # Create variant chart
            variant_data = {
                'A': country_data['variant_a'],
                'T': country_data['variant_t']
            }
            
            # Simple text display of the variant data
            st.markdown(f"""
